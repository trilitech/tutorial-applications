...
export default function MintPage() {
  ...
  useEffect(() => {
    if (storage &&   storage.extension.administrators.indexOf(userAddress! as address) < 0)
      setFormOpen(false);
  ...
  useEffect(() => {
    (async () => {
      if (nftContractTokenMetadataMap && nftContractTokenMetadataMap.size > 0) {
        formik.setFieldValue('token_id', nftContractTokenMetadataMap.size);
      }
    })();
  }, [nftContractTokenMetadataMap?.size]);
  ...
  return (
    <Paper>
      ...
      {storage ? (
        <Button
          disabled={  storage.extension.administrators.indexOf(userAddress! as address) < 0}
          sx={{
            p: 1,
            position: "absolute",
            right: "0",
      ...
      {nftContractTokenMetadataMap.size != 0 ? (
        ...
        <SwipeableViews
          ...
          enableMouseEvents
        >
          {Array.from(nftContractTokenMetadataMap!.entries()).map(
            ([token_id, token]) => (
              <Card
                sx={{
                  display: 'block',
        ...
        <MobileStepper
          variant="text"
          steps={Array.from(nftContractTokenMetadataMap!.entries()).length}
          position="static"
          ...
          nextButton={
            <Button
              ...
              disabled={
                activeStep ===
                Array.from(nftContractTokenMetadataMap!.entries()).length - 1
              }
            >
              Next
              <KeyboardArrowRight />
        ...
        <Typography sx={{ py: "2em" }} variant="h4">
          No NFTs yet. Click "Connect wallet" and mint and NFT.
        </Typography>
      )}
    </Paper>
  );
